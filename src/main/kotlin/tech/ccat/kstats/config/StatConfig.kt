...
class StatConfig(private val config: ConfigurationSection) {
    private val defaultWisdom: Double = config.getDouble("defaults.wisdom", 100.0)
    ...
    fun getDefaultStats() = PlayerStat(
        ...
        manaRegen = config.getDouble("defaults.mana-regen", 0.0)
    )

    fun getDefaultWisdom(): Double = defaultWisdom
